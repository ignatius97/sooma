    ...
    <form action="{{  Setting::get('admin_delete_control') == YES ? '#' : route('admin.subjects.save') }}" method="POST" enctype="multipart/form-data" role="form">

        <div class="box-body">

            <input type="hidden" name="subject_id" value="{{$curriculum_details->id}}">


             <div class="col-sm-6">
                <div class="form-group">
                    <label for="name" class="control-label">Subject*</label>
                    <input type="text" required class="form-control" id="name" name="subject_name" placeholder="Subject Name" minlength="3" title="Min length must be an 6 character" value="{{$curriculum_details->name }}">
                </div>
            </div>
